...
const VideoInfo = (props) => {
  ...
  return (
    <div class="info">
      <div>
        <h3>{props.video.author.username}</h3>
        <h4>{props.video.author.name}</h4>
      </div>
      <button className={following ? "active" : ""} onClick={buttonClickHandler}>{ following ? "Following" : "Follow" }</button>
      <div>
        {props.video.title}{" "}
        <span>#{props.video.hashtags[0]}</span>
      </div>
      <div>
        <span>
          <svg
            ...
            width="1em"
            height="1em"
            class="tiktok-812w79-SvgIcon eofn35l1"
          >
            <path
              fill-rule="evenodd"
              clip-rule="evenodd"
